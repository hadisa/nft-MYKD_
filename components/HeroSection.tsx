...
import { Trophy, Users, DollarSign, Globe, ArrowRight, Play } from 'lucide-react';
...
const AnimatedBackground = () => {
  ...
  return (
    <div ref={backgroundRef} className="absolute inset-0 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-slate-900/80 via-slate-800/60 to-green-900/30" />
      ...
      <FloatingElement delay={4}>
        <div className="bottom-40 left-1/4 w-14 h-14 bg-gradient-to-br from-green-500/20 to-green-700/20 backdrop-blur-sm border border-green-500/30 rounded-xl flex items-center justify-center shadow-lg hover:shadow-green-500/25 hover:scale-110 transition-transform">
          <div className="w-7 h-7 border-2 border-green-400 rounded-lg animate-spin" style={{ animationDuration: '3s' }} />
        </div>
      </FloatingElement>
      ...
      <div 
        ...
        style={{
          ...
          opacity: Math.max(0.3, 0.5 - (Math.abs(mousePosition.x - 50) + Math.abs(mousePosition.y - 50)) / 200)
        }}
      />
    </div>
  );
...
const AnimatedCounter = ({ value, label, icon: Icon, delay }: { value: string; label: string; icon: any; delay: number }) => {
  const [count, setCount] = useState(0);
  const [isVisible, setIsVisible] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={ref} className="group text-center">
      <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 hover:border-green-400/50 transition-all duration-500 hover:shadow-xl hover:shadow-green-500/10 transform hover:-translate-y-1">
        <div className="bg-green-500/20 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-green-500/30 group-hover:scale-110 transition-all duration-300">
          <Icon className="w-8 h-8 text-green-400" />
        </div>
...
const CTAButton = ({ 
  ...
  const baseStyles = "group px-8 py-4 font-semibold rounded-lg transition-all duration-300 transform flex items-center justify-center space-x-2 relative overflow-hidden";
  
  const variants = {
    primary: "bg-gradient-to-r from-green-500 to-green-600 hover:from-green-400 hover:to-green-500 text-white hover:scale-105 hover:shadow-2xl hover:shadow-green-500/25",
    secondary: "border-2 border-green-400 text-green-400 hover:bg-green-400 hover:text-slate-900"
  };
